...
def predict(net, datatot, bvalues, arg, SAMPLES=50):
    tstart = time.perf_counter()
    paramsNN = np.array([deep_simpl.predict_IVIM(datatot, bvalues, net, arg) for _ in range(SAMPLES)])
    tend = time.perf_counter()

    print(f"Took {SAMPLES} samples in {tend - tstart:.6f}s")
    print(paramsNN.shape)
    return paramsNN
...
if not arg.train_pars.skip_net:
    ...
    net = deep_simpl.learn_IVIM(datatot[res], bvalues, arg, epochs=EPOCHS)
    elapsed_time1net = time.perf_counter() - start_time
    print('\ntime elapsed for Net: {}\n'.format(elapsed_time1net))

    # Save the network
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    torch.save(net, f"./models/{timestamp}_{len(bvalues)}bvs_bnn_net_{EPOCHS}epochs_patient.pt")

    # predict parameters
    start_time = time.perf_counter()
    paramsNN_samples = predict(net, datatot, bvalues, arg)

    print("Finished sampling for images -", paramsNN_samples.shape)

    paramsNN = np.mean(paramsNN_samples, axis=1)
    cofv = np.std(paramsNN_samples, axis=1) / paramsNN

    elapsed_time1netinf = time.perf_counter() - start_time
    ...
    for k in range(len(names)):
        ...
        cofv_img = np.zeros([sx * sy * sz])
        cofv_img[valid_id] = cofv[k][tot:(tot + sum(valid_id))]
        cofv_img = np.reshape(cofv_img, [sx, sy, sz])
        nib.save(nib.Nifti1Image(cofv_img, data.affine, data.header),'{folder}/{name}_coef-var.nii.gz'.format(folder = folder,name=names[k])),

        print(f'{names[k]} => Coef of Var: (mean={np.mean(cofv[k])}, med={np.median(cofv[k])})')
